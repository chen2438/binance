...
- `scripts/run_baselines.py`、`scripts/run_cross_sectional.py` — 基线与横截面测量脚本。
...
时序信号问"这个标的在涨吗"，因此每个仓位都扛着全市场方向；横截面信号问"它相对同类在涨吗"，多空配对抵消掉大部分共同方向。在加密永续这种高度同涨同跌的市场里，这个差别通常比信号本身还大。

横截面信号实现在 `screen/cross.py` 的 `long_short_pool`，因为它本质上就是对时点特征面板做排序——正是筛选层已经在做的事。执行端 `FollowPoolSide` 只按分配到的方向下单，因此 `compute_features` 的时点保证原样继承；若让策略自己重算排名，就得额外信任它不偷看。

结果（全宇宙 473 标的，每腿 5 个，base 成本）：

| 假设 | 年化 Sharpe | 总收益 | 最大回撤 |
|---|---|---|---|
| 横截面动量 | +0.77 | +88.5% | -37.1% |
| 横截面反转 | -1.75 | -73.8% | -76.2% |
| 横截面资金费 | -0.22 | -27.7% | -60.3% |

**横截面动量是目前唯一在四档成本下都为正的读数**（stress 档仍有 +65.1%）。但拆开后它不是动量效应：
...
**81% 的利润来自资金费**，且集中在多头腿（7,164 中的 6,345）。多头腿是高动量标的，其入选当日 funding 均值为 -0.0035 而中位数为 +0.0003——均值被极端负值拉走。全样本 478,793 个标的-日中，绝对值超过 1% 的 8h 结算只有 622 条（0.118%），涉及 83 个标的，其中 602 条为负。这些是真实的轧空事件（如 ALPACAUSDT 2025-04-29 的 -4.0%），不是数据错误。
...
|---|---|---|---|
| 原始 | +88.5% | 0.77 | -37.1% |
| 资金费截尾 ±0.5%/日 | +25.6% | 0.36 | -39.2% |
| 资金费置零（纯价格信号） | **+9.6%** | **0.22** | -43.0% |

把资金费限制在仍然宽松的 ±0.5%/日，收益从 +88.5% 掉到 +25.6%；完全剔除资金费后，横截面动量信号本身只剩年化约 2.6%、Sharpe 0.22、回撤 -43%。加上最赚的 3 个标的贡献了 108% 的利润（其余净亏），以及分年收益 -1.8% / +1.3% / +53.7% / +23.3% 高度集中在后两年，**结论是这不是可用的横截面动量边际，而是对罕见轧空资金费尖峰的暴露**。

横截面结构相对时序确实有改善（时序动量全宇宙为 -0.43，横截面纯价格为 +0.22），但改善幅度不足以覆盖成本与回撤。

**方法学注记**：本节的 DSR 不可直接解读。横截面反转是横截面动量的精确镜像（同一特征，`reverse=True`），两者近乎完全负相关，而通缩公式假定试验相互独立。把互为镜像的假设当作独立试验会虚增试验间方差、抬高运气门槛。
