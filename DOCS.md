...
### 结果（2023-01..2026-06，日线，58 个标的，base 成本）

| 假设 | 年化 Sharpe | 总收益 | 最大回撤 | PSR | DSR |
|---|---|---|---|---|---|
| 动量延续 | -0.67 | -43.2% | -46.2% | 0.102 | 0.002 |
| 均值回复 | -1.56 | -67.7% | -69.2% | 0.001 | 0.000 |
| 资金费率套利 | +0.41 | +18.1% | -33.8% | 0.779 | 0.207 |

**三个假设在这个样本上都不成立。** 动量与均值回复在所有成本档位下均为负；资金费率套利名义为正，但 DSR 0.21 表明无法与运气区分。
...
- **86% 的利润来自 55 个标的中的 3 个**（ACTUSDT 10 笔交易贡献 693，AGTUSDT 6 笔贡献 361）。少数几笔小市值标的的方向性大幅波动，不是系统性的 carry 效应。
- **多头 179 笔赚 1,447，空头 653 笔只赚 360。** 利润由占少数的多头主导，而多头只在资金费为负时触发——那通常是急跌之后。这实际上是在赌反弹，不是收 carry。
- 分年收益 +9.2% / **-19.6%** / +27.8% / +6.3%，不稳定。

### 样本局限（重要）

**这个样本不能代表市场。** 已采集的 69 个标的是按字母序取的（`0GUSDT`..`BLZUSDT`），占全宇宙 787 个的 9%，且**不含任何主流币**——BTC、ETH、SOL、XRP、BNB、DOGE 全部不在样本内。

因此上述结论只适用于字母序靠前的中小市值山寨币。动量在这个样本上为负，**不能**推断动量在主流币上无效。全宇宙日线数据正在采集，完成后需要重跑。
