...
- `scripts/run_baselines.py`、`scripts/run_cross_sectional.py`、`scripts/run_funding_spike.py` — 基线、横截面与资金费尖峰测量脚本。
...
横截面动量的利润被查明来自罕见的极端负资金费事件（轧空）。本节研究能否**事前**把这个
暴露变成有意的策略：从当时可得的数据识别尖峰，次日进场。结论是不能。

**事件研究**（日 funding <= -1%，3647 次事件）。对每次事件看**次日**的价格与资金费，
即事后进场的仓位真正能捕获的收益：
...
均值为正但中位为负，且剔除最好 5% 的事件后均值翻负——正的期望完全由少数右尾事件（轧空
继续暴涨）驱动。这是一张正偏彩票：多数时候小亏，偶尔大赚。

**流动性不是瓶颈**：事件当日成交额中位数 1.43 亿美元，96% 超过 1000 万。这推翻了先前
"多为小市值、容量受限"的猜测——真正的障碍是收益分布，不是能不能进场。

**事前回测**（次日进场，完整成本扫描，422 个曾有事件的标的）：

| 场景 | 总收益 | 年化 Sharpe | 最大回撤 | 胜率 | DSR | 最短可信记录 |
|---|---|---|---|---|---|---|
| optimistic | +12.6% | 0.27 | -29.9% | 36% | 0.70 | 13,138 天 |
| base | +10.9% | 0.25 | -30.3% | 36% | 0.68 | 15,684 天 |
| stress | +4.2% | 0.16 | -31.8% | 36% | 0.62 | 39,289 天 |

**决定性证据是最短可信记录长度**：即使在乐观档，要让这个 Sharpe 在统计上区别于零，需要
约 **13,000 天（36 年）** 数据，而实有 1,276 天。样本远不足以支撑任何边际主张。三年半
换来 base 档年化约 2.9%、回撤 30%、胜率 36%——风险收益比劣于直接持有 BTC。

一个绕不过的陷阱：把止损放到极宽（100xATR）会得到 Sharpe 0.57、回撤 -0.8% 的漂亮数字，
但那是**仓位规模假象**——`size_for_risk` 按止损距离补保证金，止损极宽时数量趋近于零，
中位杠杆 0.1x、中位名义仅占权益 3%，三年半总收益仅 +1.2%。高 Sharpe 建在近乎零的敞口
上，经济意义为零。这正是 `minimum_track_record_length` 与经济显著性检验要抓的东西。

**结论：作为独立方向性策略排除。** 它更适合作为风险提示而非 alpha——极端负资金费意味着
轧空风险，据此**避免在此时做空**，比据此做多更有价值。复现见 `scripts/run_funding_spike.py`。
